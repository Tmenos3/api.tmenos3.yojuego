...
jest.mock('../../../src/repositories/PlayerESRepository', () => {
    return {
        PlayerESRepository: jest.fn((client) => {
            return {
                getBy: jest.fn((criteria) => {
                    return new Promise((resolve, reject) => {
                        resolve([{ mail: 'anyMail' }]);
                    });
                })
            }
        })
...
describe('SignUpRoutes', () => {
    ...
    it('when signUp local it must return statusCode 400 if mail exist', () => {
        ...
        //var repo = require('../repositories/PlayerESRepository');
        let req = {};
        let signUpRoutes = new SignUpRoutes();
        signUpRoutes._signUpLocal({}, 'anyMail', 'password', (err, user) => {
            expect(err.code).toEqual(400);
            expect(err.message).toEqual('La cuenta está en uso');
            expect(req.statusCode).toEqual(400);
            expect(req.statusMessage).toEqual('La cuenta está en uso');
            expect(repo.getBy.mock.calls[0][1].account.mail).toEqual('anyMail');
        });
    });
});